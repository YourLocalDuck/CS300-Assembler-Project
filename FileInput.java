...
public class FileInput {
    ...
    // If a parsed instruction for the current filename already exists, return that. Otherwise, parse the file and create a parsed instruction set.
    ...
    // Parsing the instruction by breaking it into a label, mnemonic, operands, and comment. Skips lines that are empty or start with '.' The fields label, mnemonic, operands, and comment are assumed to be separated by a '  '(tab). Also, depending on whether each line has 2 or less tab separated fields, operands or comment is left blank. 
    ...
    // Setter for filename, and also clears parsedInstructions, so that getParsedInstructions() will reset properly, as the old parsed data, if it exists, will be irrelevant.
